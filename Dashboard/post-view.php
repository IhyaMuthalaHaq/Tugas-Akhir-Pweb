                    ...
                    <div class="table-responsive">
                        <table id="myDataTable" class="table table-bordered table-stripe">
                            ...
                            <tbody>
                                ...
                                $posts = "SELECT p.*, c.name AS cname FROM post p,categories c WHERE c.category_id = p.category_id ";
                                $posts_run = mysqli_query($con, $posts);

                                if (mysqli_num_rows($posts_run) > 0) {
                                    foreach ($posts_run as $post) { // Add missing opening parenthesis here
                                ?>
                                        <tr>
                                            <td><?= $post['id'] ?></td>
                                            <td><?= $post['name'] ?></td>
